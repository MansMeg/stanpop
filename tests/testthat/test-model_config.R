...
test_that("model_config works", {
  ...
  expect_message(mc1 <- model_config(model = "model8k2", x = cfg1))
  expect_message(mc2 <- model_config(model = "model8k2", x = NULL))
  expect_message(mc3 <- model_config(model = "model8k2", x = cfg2))
  expect_message(mc4 <- model_config(model = "model8k2", x = cfg3))
  expect_error(model_config(model = "model99"))
  expect_identical(mc1, mc4)

  expect_message(mc5 <- model_config(model = "model8k2", cfg5))
...
test_that("use_..._bias is needed for use_constrained_..._bias", {
  ...
  cfg1 <- list("sigma_kappa_hyper" = 0.01,
               ...
               g_scale = 4)
  expect_message(mc1 <- model_config(model = "model8k2", cfg1))

  cfg1$use_industry_bias <- 0L
  expect_error(mc1 <- suppressMessages(model_config(model = "model8k2", cfg1)))
...
test_that("g_scale is computed correctly", {
  cfg1 <- list(use_house_bias = 1L)
  expect_error(mc1 <- suppressMessages(model_config(model = "model8k2", cfg1, stan_data = 1L)))
