...
test_that("Test model 8f1 and 8f3 are identical", {
 ...
 #              use_constrained_party_house_bias = 1L,
  ...
  expect_equal(lp1_8f3, lp1_8f1)

  set.seed(4712)
  par_values <- rnorm(no_up)
  lp2_8f3 <- log_prob(pop8f3, par_values)
  lp2_8f1  <- log_prob(pop8f1, par_values)
  expect_equal(lp2_8f3, lp2_8f1)


  # Check that including AR makes a difference (due the new prior)
  cfg <-  list(sigma_kappa_hyper = 0.03,
               use_industry_bias = 1L,
               use_house_bias = 0L,
               use_design_effects = 0L,
  ...
  lp1_8f3 <- log_prob(pop8f3, par_values)
  lp1_8f1  <- log_prob(pop8f1, par_values)
  expect_lt(lp1_8f3, lp1_8f1)

  set.seed(4712)
  par_values <- rnorm(no_up)
  lp2_8f3 <- log_prob(pop8f3, par_values)
  lp2_8f1  <- log_prob(pop8f1, par_values)
  expect_lt(lp2_8f3, lp2_8f1)
})
...
test_that("Visualize results", {
  ...
  cfg$nu_kappa_raw_beta <- 0.5

  expect_silent(pop8f3_out <-
                  capture.output(
                    suppressWarnings(
                      suppressMessages(
                        pop8f3t <- poll_of_polls(y = parties,
                                               model = "model8f3",
                                               polls_data = spd,
                                               time_scale = time_scale,
                                               known_state = known_state,
  ...
  cfg$nu_kappa_raw_beta <- 1.0

  expect_silent(pop8f3_out <-
                  capture.output(
                    suppressWarnings(
                      suppressMessages(
                        pop8f3nt <- poll_of_polls(y = parties,
                                                 model = "model8f3",
                                                 polls_data = spd,
                                                 time_scale = time_scale,
                                                 known_state = known_state,
...
test_that("Check negative values are truncated", {
  skip_if_not(adapop:::test_stan_full_on_local() | adapop:::on_github_actions_test_branch())
  ...
  kappa_x4_s1 <- 0.03 * (1/g_scale)
  kappa_x4_s2 <- 0.025 * (1/g_scale)
  kappa_x4_s3 <- 0.03 * (1/g_scale)
  kappa_x4_s4 <- 0.025 * (1/g_scale)
  kappa_x4_s5 <- 0.0 * (1/g_scale)
  ...
  expect_silent(pop8f1_out <-
                  capture.output(
                    suppressWarnings(
                      suppressMessages(
                        pop8f2_bz <- poll_of_polls(y = parties,
                                                   model = "model8f2",
                                                   polls_data = spd,
                                                   time_scale = time_scale,
                                                   known_state = known_state,
  ...
  expect_silent(pop8f1_out <-
                  capture.output(
                    suppressWarnings(
                      suppressMessages(
                        pop8f3_bz <- poll_of_polls(y = parties,
                                                model = "model8f3",
                                                polls_data = spd,
                                                time_scale = time_scale,
                                                known_state = known_state,
  ...
  expect_silent(pop8f3_out <-
                  capture.output(
                    suppressWarnings(
                      suppressMessages(
                        pop8f3t_bz <- poll_of_polls(y = parties,
                                                model = "model8f3",
                                                polls_data = spd,
                                                time_scale = time_scale,
                                                known_state = known_state,
