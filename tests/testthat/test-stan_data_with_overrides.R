...
test_that("stan_polls_data attaches an override-aware path without changing legacy data", {
  ...
  mtr <- time_range(spd)
  override_from <- as.Date(mtr["from"]) + 20
  override_to <- override_from + 4
...
test_that("parallel override-aware path respects model-specific known_state filtering", {
  data("x_test")
  txdf <- as.data.frame(x_test[3:4])
  colnames(txdf) <- paste0("x", 3:length(x_test))
  data("pd_test")
  ...
  in_range_date <- as.Date(time_range(spd)["from"]) + 35
  known_state <- tibble::tibble(
    date = as.Date(c("2009-12-25", as.character(in_range_date))),
    x3 = c(0.25, 0.30),
    x4 = c(0.20, 0.22)
  )
