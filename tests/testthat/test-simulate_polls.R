...
test_that("reweight and resample works", {
  ...
  reweight_and_resample <- get_internal("reweight_and_resample")
  expect_silent(spd2 <- reweight_and_resample(spd1, x_test[[1]], time_scale = "week"))
  expect_identical(spd1,spd2)

  expect_silent(spd3 <- reweight_and_resample(x = spd1, true_ls = x_test[[1]], time_scale = "week", weight = "keep"))
  expect_failure(expect_identical(spd1, spd3))
  expect_identical(ptw1, polls_time_weights(spd3))

  expect_silent(spd4 <- reweight_and_resample(spd1, x_test[[1]], time_scale = "week", weight = "skew"))
  ...
  expect_silent(spd5 <- reweight_and_resample(spd1, x_test[[1]], time_scale = "week", weight = "vskew"))
