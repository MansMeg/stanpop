...
data {
  ...
  int<lower=0, upper=1> use_conditional_model;
  int H_cond;
  array[use_conditional_model ? H: 0] int houses_key;
  row_vector[use_conditional_model? P : 1] eta_1_mu_hyper;
   row_vector[use_conditional_model? P : 1] eta_1_sigma_hyper;
   vector[use_conditional_model? P: 1]  kappa_1_mu_hyper;
   array[use_conditional_model? P: 1]  real<lower=0> kappa_1_sigma_hyper;
   vector[use_conditional_model ? H: 1] beta_sigma_1_mu_hyper;
   array[use_conditional_model ? H: 1] real<lower=0> beta_sigma_1_sigma_hyper;
   array[use_conditional_model ? H_cond : 1, use_conditional_model ? P : 1] real beta_mu_1_mu_hyper;
   array[use_conditional_model ? H_cond : 1, use_conditional_model ? P : 1] real<lower=0> beta_mu_1_sigma_hyper;
 ...
 matrix[T_known, P] x_known; // known x
...
transformed data {
 ...
 array[T] int x_t_is_known =  rep_array(0, T);
  ...
  vector[P]  kappa_1_mean_hyper;
  array[P] real<lower=0>  kappa_1_sd_hyper;
  vector[H] beta_sigma_1_mean_hyper;
  array[H] real<lower=0> beta_sigma_1_sd_hyper;
  int H_max = use_conditional_model ? H_cond : H;
  array[H_max,P] real beta_mu_1_mean_hyper;
  array[H_max, P] real beta_mu_1_sd_hyper;

  // Conditional model
  array[N] int<lower=1, upper=H_max> H_i;

  if (use_conditional_model){
    kappa_1_mean_hyper = kappa_1_mu_hyper;
    kappa_1_sd_hyper = kappa_1_sigma_hyper;
    beta_sigma_1_mean_hyper = beta_sigma_1_mu_hyper;
    beta_sigma_1_sd_hyper = beta_sigma_1_sigma_hyper;
    beta_mu_1_mean_hyper = beta_mu_1_mu_hyper;
    beta_mu_1_sd_hyper = beta_mu_1_sigma_hyper;
    for (i in 1:N){
      H_i[i] = houses_key[h_i[i]];
    }
  }else{
    kappa_1_mean_hyper = rep_vector(kappa_1_mu_hyper[1], P);
    kappa_1_sd_hyper = rep_array(kappa_1_sigma_hyper[1], P);
    beta_sigma_1_mean_hyper = rep_vector(beta_sigma_1_mu_hyper[1], H);
    beta_sigma_1_sd_hyper = rep_array(beta_sigma_1_sigma_hyper[1], H);
    beta_mu_1_mean_hyper = rep_array(beta_mu_1_mu_hyper[1,1], H, P);
    beta_mu_1_sd_hyper = rep_array(beta_mu_1_sigma_hyper[1,1], H, P);
    H_i = h_i;
  }
  ...
  // x_{t=1} prior (Dirichlet(x1_prior_p, x1_prior_alpha0))
    if (use_conditional_model){
      t1_prior_mu = eta_1_mu_hyper;
      t1_prior_sigma = eta_1_sigma_hyper;
    } else{
...
parameters {
  ...
  array[use_house_bias ? S : 0, use_house_bias ? H_max : 0, use_house_bias ? P : 0] real beta_mu;
...
transformed parameters {
  ...
  matrix[use_constrained_party_house_bias ? S : 0, use_constrained_party_house_bias ? H_max : 0] beta_mu_sum_H = rep_matrix(0, use_constrained_party_house_bias ? S : 0, use_constrained_party_house_bias ? H_max : 0);
  ...
  if(use_industry_bias){
    ...
    for(p in 1:P){
      ...
      if(use_ar_kappa){
        if(use_t_dist_industry_bias){
          kappa[1, p] = sqrt(v_kappa[1]) * kappa_raw[1, p] * kappa_1_sd_hyper[p];
          for(j in 2:no_unknown_kappa)
            kappa[j, p] = alpha_kappa[1] * kappa[j-1, p] + sqrt(v_kappa[1]) * kappa_raw[j, p] * sigma_kappa[p];
        } else {
          kappa[1, p] = kappa_raw[1, p] * kappa_1_sd_hyper[p] + kappa_1_mean_hyper[p];
          for(j in 2:no_unknown_kappa)
...
//    equivalent to: target += multi_normal_cholesky_lpdf(eta[t, ] | eta[t-1, ], diag_pre_multiply(sigma_latent, L_Omega[s_t_Omega[t]]));
  ...
  if(use_house_bias){
    for(p in 1:P){
      for(h in 1:H_max) {
        target += normal_lpdf(beta_mu[1,h,p] | beta_mu_1_mean_hyper[h,p], beta_mu_1_sd_hyper[h,p]);
        if(S > 1){
          for(s in 2:S) {
  ...
  if(use_design_effects){
    for(h in 1:H) {
      target += normal_lpdf(beta_sigma[1,h] | beta_sigma_1_mean_hyper[h], beta_sigma_1_sd_hyper[h]);
      if(S > 1){
        for(s in 2:S) {
