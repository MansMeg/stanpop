    ...
    for(t in t_start_all:t_end_all){
      if(x_t_is_known[t]){ // then eta_z_t is also known (see derivation)
        if(election_period[t] > 0){
          if(use_sigma_ep == 1)
            L_Sigma_ep[1] = diag_pre_multiply((exp(-to_vector(eta[t-1,])) * sigma_ep[1] + sigma_x), L_Omega_x[s_t_Omega[t]]);
          if(use_sigma_ep == 2)
            L_Sigma_ep[1] = diag_pre_multiply((exp(-to_vector(eta[t-1,])) .* sigma_ep + sigma_x), L_Omega_x[s_t_Omega[t]]);
