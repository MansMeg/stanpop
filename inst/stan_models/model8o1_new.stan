...
data {
  ...
  int<lower=0, upper=2> use_sigma_ep;
  int<lower=0> election_period[T]; // indicator if by time point
  int<lower=0> no_ep; // number of election periods
  int<lower=0> ep_length[no_ep]; // the length of each election period
  int<lower=0> epi[T]; // index for election period time point

  real alpha_V_mean; // prior mean for N+ prior
  real<lower=0> alpha_V_sd; // prior sd for N+ prior
}
...
parameters {
  ...
  matrix<lower=0>[use_jump_process ? P : 0, use_jump_process ? ep_length : 0] V[use_jump_process ? no_ep : 0];
  vector<lower=2>[use_jump_process ? P : 0] alpha_V; //shape of jumps
    ...
    for(t in t_start_all:t_end_all){
      if(x_t_is_known[t]){ // then eta_z_t is also known (see derivation)
        if(election_period[t] > 0){
          if(use_sigma_ep == 1)
            L_Sigma_ep[1] = diag_pre_multiply((exp_logsumexp_minus_eta(eta[t-1,]) * sigma_ep[1] + sigma_x), L_Omega_x[s_t_Omega[t]]);
          if(use_sigma_ep == 2)
            L_Sigma_ep[1] = diag_pre_multiply((exp_logsumexp_minus_eta(eta[t-1,]) .* sigma_ep + sigma_x), L_Omega_x[s_t_Omega[t]]);
