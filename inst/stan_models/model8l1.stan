...
transformed parameters {
  ...
  cholesky_factor_cov[use_multivariate_model ? P : 0] L_Sigma_ep[no_Omega_ep];
  ...
  for(i in 1:no_Omega){
    ...
    if(use_sigma_ep == 1)
      L_Sigma_ep[i] = sigma_ep[1] * L_Sigma[i];
    if(use_sigma_ep == 2)
      L_Sigma_ep[i] = diag_pre_multiply(sigma_ep, L_Sigma[i]);
  }
    ...
    for(t in t_start_all:t_end_all){
      if(x_t_is_known[t]){ // then eta_z_t is also known (see derivation)
        if(election_period[t] > 0){
          eta_z[t,] = to_row_vector(inverse(L_Sigma_ep[s_t_Omega[t]]) * to_vector((eta[t,] - eta[t-1,])));
        } else {
          eta_z[t,] = to_row_vector(inverse(L_Sigma[s_t_Omega[t]]) * to_vector((eta[t,] - eta[t-1,])));
        }
      } else {
        if(election_period[t] > 0){
          eta[t,] = eta[t-1,] + to_row_vector(L_Sigma_ep[s_t_Omega[t]] * to_vector(eta_z[t,]));
        } else {
