...
data {
  ...
  // 2 = symmetric NIG
  // 3 = non-symmetric NIG
  int<lower=0, upper=3> use_industry_bias_dist;
...
parameters {
  ...
  vector<lower=0>[use_industry_bias_dist == 1 ? 1 : 0] nu_kappa_raw;
  vector<lower=0>[use_industry_bias_dist == 1 ? 1 : 0] v_kappa;
...
transformed parameters {
  ...
  vector<lower=1>[use_industry_bias_dist == 1 ? 1 : 0] nu_kappa = rep_vector(2, use_industry_bias_dist == 1 ? 1 : 0);
  ...
  if(use_industry_bias){
    if(use_industry_bias_dist > 1){
      ...
      if(use_industry_bias_dist == 1){
        ...
        target += inv_gaussian_lpdf(v_kappa | nu_kappa, nu_kappa - 1);
      }
    }
    for(p in 1:P){
      for(j in 1:no_unknown_kappa) {
      ...
      if(estimate_kappa_next == 0){
        ...
        for(p in 1:P){
          if(estimate_alpha_kappa){
            ...
            } else if(use_industry_bias_dist > 1){
              fix!!!
              kappa_pred[(T_known + 1), p] = nig_rng(2 * nu_kappa[1], alpha_kappa[1] * kappa[no_unknown_kappa, p], sqrt(0.5 * (nu_kappa[1] - 1) / nu_kappa[1]) * sigma_kappa[p]);
            } else {
