...
parameters {
  ...
  real beta_mu_z[use_house_bias ? S : 0, use_house_bias ? H : 0, use_house_bias ? P : 0];
  real<lower=0> sigma_beta_mu[use_house_bias ? 1 : 0];
  real beta_sigma_z[use_design_effects ? S : 0, use_design_effects ? H : 0];
...
transformed parameters {
  ...
  // Non-centered parametrization of beta_mu
  ...
  if(use_house_bias){
    for(p in 1:P){
      for(h in 1:H) {
        ...
        beta_mu[1, h, p] = beta_mu_z[1, h, p];
        if(S > 1){
          for(s in 2:S) {
            beta_mu[s, h, p] = alpha_beta_mu[1] * beta_mu[s - 1, h, p] + sigma_beta_mu[1] * beta_mu_z[s, h, p];
            // equivalent to:
            // target += normal_lpdf(beta_mu[s, h, p] | alpha_beta_mu[1] * beta_mu[s - 1, h, p], sigma_beta_mu);
          }
        }
  ...
  if(use_design_effects){
    for(h in 1:H) {
      ...
      beta_sigma[1, h] = beta_sigma_z[1, h];
      if(S > 1){
        for(s in 2:S) {
          // equivalent to:
          // target += normal_lpdf(beta_sigma[s, h] | alpha_beta_sigma[1] * beta_sigma[s - 1, h], sigma_beta_sigma);
          beta_sigma[s, h] = alpha_beta_sigma[1] * beta_sigma[s - 1, h] + sigma_beta_sigma[1] * beta_sigma_z[s, h];
        }
      }
    }
  }
...
//    equivalent to: target += multi_normal_cholesky_lpdf(eta[t, ] | eta[t-1, ], diag_pre_multiply(sigma_latent, L_Omega[s_t_Omega[t]]));
  ...
  if(use_house_bias){
    for(p in 1:P){
      for(h in 1:H) {
          target += normal_lpdf(beta_mu_z[1, h, p]| 0, beta_mu_1_sigma_hyper);
          for(s in 2:S) {
            target += std_normal_lpdf(beta_mu_z[s, h, p]);
            // equivalent to:
            // target += normal_lpdf(beta_mu[s, h, p] | alpha_beta_mu[1] * beta_mu[s - 1, h, p], sigma_beta_mu);
          }
        }
      }
    // Prior on sigma_beta_mu
  ...
  if(use_design_effects){
    for(h in 1:H) {
        target += normal_lpdf(beta_sigma_z[1, h]| 0, beta_sigma_1_sigma_hyper);
        for(s in 2:S) {
          target += std_normal_lpdf(beta_sigma_z[s, h]);
            // equivalent to:
            // target += normal_lpdf(beta_sigma[s, h] | alpha_beta_sigma[1] * beta_sigma[s - 1, h], sigma_beta_sigma);
        }
      }
    // Prior for sigma_beta_sigma
