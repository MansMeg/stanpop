...
#' @param parameters Paramaters to the individual report. See details.
...
#'
#' @details
#' Each report has its own parameter settings.
#' See object param_default in the individual rmd report.
#'
#' @export
write_report <- function(..., report, parameters = NULL, output_file = NULL, output_format = 'pdf_document', output_dir = getwd()){
  ...
  if(file.exists(report)){
    ...
    rmd_path <- system.file(file.path("reports", report), package = "stanpop")
  }
...
supported_reports <- function(){
  dir(system.file(file.path("reports"), package = "stanpop"))
}
...
rmd_model_settings <- function(pops, unique_only = TRUE){
  ...
  df$election_period <- unlist(lapply(pops, function(x) format_election_period_to_string(x, "--", ", ")))
  pops <- lapply(pops, function(x) {x$model_arguments$election_period <- NULL;return(x)})

  df$obs_x <- unlist(lapply(pops, function(x) format_obs_x_to_string(x, ", ")))
  pops <- lapply(pops, function(x) {x$model_arguments$obs_x <- NULL;return(x)})

  df$ep_inv_x <- unlist(lapply(pops, function(x) format_ep_inv_x_to_string(x, ", ")))
  pops <- lapply(pops, function(x) {x$model_arguments$ep_inv_x <- NULL;return(x)})

  for(j in seq_along(pops)){
    for(i in seq_along(pops[[j]]$model_arguments)){
      if(length(pops[[j]]$model_arguments[[i]]) > 1) {
        pops[[j]]$model_arguments[[i]] <- paste(pops[[j]]$model_arguments[[i]], collapse = ", ")
      }
    }
  }
...
rmd_parameter_table <- function(x, pars, par_names, digits = 3, ...){
  res <- rstan::summary(x$stan_fit, pars)
  rownames(res$summary) <- par_names
  knitr::kable(res$summary, digits = digits, ...)
}
